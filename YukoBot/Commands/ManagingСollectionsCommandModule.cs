...
namespace YukoBot.Commands
{
    ...
    public class ManagingСollectionsCommandModule : CommandModule
    {
        ...
        {
            await AddToCollection(commandContext, new YukoDbContext(), commandContext.Message.Reference.Message);
        }
            ...
            [Description("Id сообщения")] ulong messageId)
        {
            ...
            {
                await AddToCollection(commandContext);
            }
        }
        ...
        private async Task AddToCollection(CommandContext commandContext, YukoDbContext dbContext, DiscordMessage message)
        {
            ...
            if (isAttacments)
            {
                ...
                DbCollection dbCollection = dbContext.Collections.Where(x => x.UserId == memberId).FirstOrDefault();
