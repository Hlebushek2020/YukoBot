...
end|-|Задать вложенное сообщение конечным сообщением (входит в промежуток) и добавить входящие в промежуток сообщения в заданную коллекцию.  Если коллекция не указана сообщения добавляются в коллекцию по умолчанию
...
-|-|-|-|-|-
add-command-response|add-response|Отправка сообщения об успешности выполнения команды `add` на сервере (взамен сообщение будет приходить в ЛС)|:heavy_check_mark:|:heavy_check_mark:|:x:
...
settings|-|Показать настройки для подключения|:heavy_check_mark:|:heavy_check_mark:|:heavy_check_mark:
...
Что бы получить ссылку на актуальную версию приложения и доступ к основному функционалу бота выполняем команду `reg` бота (не забываем про префикс бота), после чего в личные сообщения придет логин и пароль (понадобятся для входа в приложения). Затем выполняем команду `app` бота и переходим по ссылке, скачиваем архив с клиентами (начинается со слова clients), распаковываем его куда вам удобно.\
Запускаем нужное нам приложение (YukoClient.exe или YukoCollectionClient.exe). Перед нами появится окно входа, вводим туда логин и пароль полученные ранее. Если мы нажмем на кнопку "Войти" нам выдастся следующее сообщение "Сначала настройте программу! Значок в правом нижнем углу", следуем этому указанию (если у вас не появляется такое сообщение или появляется другое, то все равно выполните действия описанные далее).\
